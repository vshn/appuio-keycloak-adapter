...
//+kubebuilder:rbac:groups=appuio.io,resources=users/finalizers,verbs=update
...
func (r *UserReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if !user.ObjectMeta.DeletionTimestamp.IsZero() {
		...
		// err := r.Keycloak.DeleteGroup(ctx, user.Namespace, user.Name)
		// if err != nil {
		// 	r.Recorder.Event(user, "Warning", "DeletionFailed", "Failed to delete Keycloak Group")
		// 	return ctrl.Result{}, err
		// }

		err := r.removeFinalizer(ctx, user)
		return ctrl.Result{}, err
	}
	err := r.addFinalizer(ctx, user)
	if err != nil {
		return ctrl.Result{}, err
	}
...
func (r *UserReconciler) addFinalizer(ctx context.Context, user client.Object) error {
	if !controllerutil.ContainsFinalizer(user, orgFinalizer) {
		controllerutil.AddFinalizer(user, orgFinalizer)
...
func (r *UserReconciler) removeFinalizer(ctx context.Context, user client.Object) error {
	if controllerutil.ContainsFinalizer(user, orgFinalizer) {
		controllerutil.RemoveFinalizer(user, orgFinalizer)
		if err := r.Update(ctx, user); err != nil {
			return err
		}
	}
